...
1. 나는 URL 하나를 붙여넣으면 즉시 카드가 생기고(분석 중 표시), 몇 초 뒤 제목·요약·태그가 채워진다. (썸네일 대신 파비콘 컴팩트 카드)
2. 나는 링크를 **중첩 폴더**로 정리한다 (예: `AI/Transformer`). 폴더마다 소유주가 설명을 달아둔다.
3. 나는 **폴더로 정리하고, 검색·태그로 찾는다** (정리·공유 = 폴더, 찾기 = 검색·태그 보조).
4. 나는 **폴더를 통째로 공유**해서 링크 묶음을 남에게 뿌린다. 받는 사람은 읽기 전용으로 보고 자기 서랍에 담는다(담기는 후순위).
5. 나는 AI가 단 요약/태그가 마음에 안 들면 직접 고칠 수 있다 (커스터마이징).
6. 나는 네이버 블로그나 디시 글을 저장해도 본문이 제대로 요약되어 나온다.

명시적 후순위 (스키마에 자리만 마련, 구현 안 함):

- 공개 폴더 "담기(import)" — 공유 페이지 자체는 만들되(Phase 4), 받은 폴더를 자기 서랍에 복사하는 기능은 후순위
- 크롬 익스텐션
...
### Phase 1 — 로그인 + 폴더·북마크 CRUD (AI 없음)

- server: Spring Security OAuth2 Client(Google) 콜백 → JWT access+refresh 발급/회전, 인증 필터로 API 보호
- server: **Folder 엔티티(중첩·이동·설명) + 루트 폴더 자동 생성**, Bookmark에 folderId
- web: 로그인 화면, 토큰 처리, **가운데 고정폭 레이아웃(폴더 헤더 + 폴더 그리드 + 링크 그리드)**, URL 입력 → `POST /bookmarks`(현재 폴더, PENDING) → 카드
- 폴더 생성/이동/설명 편집, 카드 편집(제목/메모/폴더 이동)·삭제, 시드 스크립트(폴더 트리 + 다양한 상태)
- **DoD**: 로그인 → 폴더 만들고 그 안에 URL 저장 → breadcrumb로 폴더 이동 → 편집/삭제가 실제 브라우저에서 동작. refresh 회전·로그아웃(블랙리스트)이 동작. 시드로 pending/done/failed 카드 + 중첩 폴더가 모두 렌더링됨.
...
### Phase 4 — 검색 · 공유 · 커스터마이징

- 검색: 제목/요약/메모/태그 대상, pg_trgm 인덱스 + ILIKE (한국어 부분일치에 실용적)
- **폴더 공유**: 폴더 PUBLIC 전환 → `shareSlug` 공개 페이지(비로그인 읽기), 설명 안내문 노출
- 폴더 이동·정렬, 태그 필터, 태그 rename/색상/병합, 카드 핀 고정
- AI 결과 수동 수정(요약/태그 편집) — 수정본이 검색 대상
- **DoD**: 시드 50개 기준 검색·태그 필터가 200ms 내 응답. 폴더를 공개하면 공유 URL이 비로그인에서 트리+링크+설명으로 열림. 태그 병합 후 카드들이 올바르게 재연결됨.

### Phase 5 — 폴리시 & 선택 과제

- 디자인 마감(빈 상태, 로딩, 에러, 반응형), 다크 모드
- (선택) pgvector + Gemini 임베딩으로 시맨틱 검색 → 키워드와 하이브리드
- (선택) 공개 폴더 "담기(import)", Reader View, 배포(Vercel + Neon/Supabase 또는 VPS)
...
5. ~~액센트 컬러~~ → ~~A안(먹+주홍)~~ → **B안 (먹 + 심록 `#1F6A4E`)**, 약한 브루탈리즘 가미 (2026-07-23 재확정). 상세는 02 문서.
...
8. **정리 구조** → 태그 평면에서 **중첩 폴더 중심 + 태그 보조**로 (2026-07-23 시안 확정). 폴더마다 소유주 편집 설명.
9. **공유** → **폴더 단위 공개 페이지**(읽기 전용, `shareSlug`). Phase 4에서 공유 페이지, 담기(import)는 후순위.
10. **레이아웃** → 사이드바 대시보드에서 **한국 웹식 가운데 고정폭(900) + 좌우 광고 레일(선택)**로. 카드 썸네일 제거(파비콘 컴팩트).
